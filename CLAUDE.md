...
- **Data (`andrew` account):** now **3 vehicles** after user cleanup — **Audi SQ5** (53 fuel), **Cadillac CT5-V Blackwing** (69 fuel), **Rocket 3** (Triumph, 27 fuel). Real history: AutoCare import (Sept 2025) + a "Car tracker" Google Sheet append (Oct 2025→2026: 30 Audi + 34 Cadillac fills, matched by odometer continuity). The other imported vehicles (GMC Sierra, Honda Goldwing, Cadillac CTS-V) and all test vehicles were deleted by the user. One-off importers ran from a gitignored `scripts/autocare-import/` (deleted after each use, incl. the token). Imports go through the live API with the user's `sb_access` token in a gitignored `.token` file.
