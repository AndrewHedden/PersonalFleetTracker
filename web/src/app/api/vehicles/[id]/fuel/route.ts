...
/**
 * GET  /api/vehicles/:id/fuel — list fuel entries for the vehicle.
 * POST /api/vehicles/:id/fuel — log a new entry.
 *
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const body = await apiFetch<ListFuelEntriesResponse>(
      token,
      `/v1/vehicles/${encodeURIComponent(id)}/fuel`,
    );
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = CreateFuelEntryInputSchema.safeParse(body);
  if (!parsed.success) {
    return jsonError(
      400,
      'invalid_input',
